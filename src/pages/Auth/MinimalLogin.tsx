...
export const MinimalLogin = () => {
    ...
    const onSubmit = async (data: LoginFormData) => {
        try {
            ...
            // IMPORTANTE: O response JÁ vem com { success, token, user, message }
            if (response.success && response.token) {
                // 🔥 buscar user completo com roles
                const me = await authService.getCurrentUser();
                const fullUser = me.user || me.data;

                if (!fullUser) {
                    throw new Error("Failed to load full user (/auth/me)");
                }

                login(fullUser, response.token, data.rememberMe);
            ...
            if (error.response?.data?.error?.toLowerCase().includes("verify")) {
            ...
            <div style={{
                ...
                <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                    ...
                    <p style={{ color: '#888', fontSize: '14px' }}>
                        Sign in to your Washington Gaming account
                    </p>
                </div>
